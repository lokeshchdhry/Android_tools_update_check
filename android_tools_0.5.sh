...
installed_NDK(){
  ...
  echo "Highest Installed NDK :"$latest_NDK_installed
}

check_avaliable_NDK(){
  installed_NDK
  url="http://developer.android.com/ndk/downloads/index.html#Downloads"
  ava_ndk=$(curl -s $url|grep "android-ndk-r"|awk -F "android-ndk" '{print $2}'|head -1|cut -c2-5)
  echo "Avaliable NDK :"$ava_ndk

  if [ "$latest_NDK_installed" != "$ava_ndk" ]; then
    echo "New NDK Avaliable :"$ava_ndk
    osascript -e 'display notification "New Android NDK Avaliable :'$ava_ndk'" with title "New Android NDK Avaliable"'
  else
    echo "No New Android NDK Avaliable"
